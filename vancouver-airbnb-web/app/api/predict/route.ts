...
// Global variable to store loaded sessions
...
async function initializeSessions() {
  ...
  try {
    ...
    const promises = modelNames.map(async (name) => {
      const modelPath = `${process.cwd()}/public/models/${name}.onnx`;
...
function getKeywordFeature(text: string, keyword: string): number {
  return text.toLowerCase().includes(keyword.toLowerCase()) ? 1 : 0;
}

function prepareFeatures(data: PredictionFormData): Float32Array {
  ...
  keywords.forEach((k) => {
    const hasKw =
      getKeywordFeature(name, k) || getKeywordFeature(desc, k);
    setF(`txt_${k}`, hasKw);
  });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const feats = prepareFeatures(data);
    ...
    // Run Inference
    // Price
    const pPoint = await sessions["Price_Point"].run(feeds);
    const pLower = await sessions["Price_Lower_q5"].run(feeds);
    const pUpper = await sessions["Price_Upper_q95"].run(feeds);

    // Revenue
    const rPoint = await sessions["Revenue_Point"].run(feeds);
    const rLower = await sessions["Revenue_Lower_q5"].run(feeds);
    const rUpper = await sessions["Revenue_Upper_q95"].run(feeds);
